      ...
      <ul class="debt-modify">
        ...
        <li v-if="job_class">
          <div v-if="job_class.value == '1'">
            <p class="key">연소득</p>
            <p>
              <money v-model="amt_year_income" v-validate="'required'" data-vv-name='금액' />만원
            </p>
          </div>
          <div v-if="job_class.value == '2'">
            <p class="key">연매출</p>
            <p>
              <money v-model="amt_year_sale" v-validate="'required'" data-vv-name='금액' />만원
            </p>
          </div>
...
export default {
  ...
  methods: {
    ...
    clickNext: function() {
      ...
      if (
        (this.job_class.value == "1" && this.amt_year_income != "") ||
        (this.job_class.value == "2" && this.amt_year_sale != "")
      ) {
        this.modifyLoanREIncomeInfo();
      }
    },
    modifyLoanREIncomeInfo: function() {
      var _this = this;
