                ...
                <form @submit.prevent="saveEvent">
                  ...
                  <section class="v-cal-dialog-card__body">
                      <div class="v-cal-fields">
                          ...
                          <template v-for="field in cFields">
                            <event-dialog-input v-model="event[field.name]" :key="field.name + '-' + field.index" :field="field" v-if="!field.cFields"/>
                            <div class="v-cal-input-group" v-else :key="field.index">
                                <label v-if="field.label">{{ field.label }}</label>
                                <event-dialog-input v-model="event[subfield.name]" :key="subfield.name + '-' + field.index" :field="subfield" v-for="subfield in field.cFields"/>
                              </div>
                          </template>
                      </div>
                  </section>
