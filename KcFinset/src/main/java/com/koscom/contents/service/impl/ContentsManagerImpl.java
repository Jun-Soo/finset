...
//		}
	...
	public ReturnClass modifyNewsManage(ContentsVO contentsVO) {
		...
		if(contentsVO.getImg_files() != null && contentsVO.getImg_files().length > 0){
			contentsVO.setImg_files(contentsVO.getImg_files());
			contentsVO.setNm_img_files(contentsVO.getNm_img_files());
			...
			if(StringUtils.isEmpty(seq_thum_file)){
				...
				if(1 != resultData){
					return new ReturnClass(Constant.FAILED, "썸네일 첨부 실패하였습니다.");
				}
			}else{
