...
public class FinanceManagerImpl implements FinanceManager {
    ...
    private TxFcTransmitVO setFcEdocData(TxFcTransmitVO fcEdocSendVO, String type){
        ...
        List<ScrRespHealthPaymentdtlVO> listScrRespHealthPaymentdtl = null;
        ...
        if (fcEdocSendVO !=null) {
            ...
            if(TRANS_1ST.equals(type)){//1차전문 값 설정
                ...
                */
                ...
                String pay_yyyymm = DateUtil.addMonths(toDay, -6);
                scrRespHealthPaymentdtlVO.setPay_yyyymm(pay_yyyymm);
                listScrRespHealthPaymentdtl = scrapManager.getScrRespHealthPaymentdtl(scrRespHealthPaymentdtlVO);


                //내역이 존재하는 경우
                
                if( listScrRespHealthPaymentdtl != null && listScrRespHealthPaymentdtl.size() > 0 ){

                    nAveCnt = listScrRespHealthPaymentdtl.size();
                    //내역 건수가 3건 이상 인 경우 최신 3건으로 설정
                    if( nAveCnt >= 3 ) {
                        nAveCnt = 3;
                    }
                    ScrRespHealthPaymentdtlVO scrRespHealthPaymentdtl = null;
                    for(int i = 0; i < nAveCnt; i++){
                    	scrRespHealthPaymentdtl = listScrRespHealthPaymentdtl.get(i);
                        nTolAmt += Long.parseLong(scrRespHealthPaymentdtl.getAmt_nt_health_insu());
                    }
                ...
                if(TYPE_FLEX_TCP.equals(type_flex)) {//TCP
                    ...
                    if(txFcRecvVO != null) {
                        ...
                        if(ApplyVO.CD_APPLY_DOC_BOX_70.equals(sts_loan)){
                            ...
                            if( sDenyCode != null ) {
                            	StdCodeInfo stdCodeInfo = stdCodeManager.getStdCodeInfo("reason", sDenyCode);
                            	if(stdCodeInfo != null)	{
                            		sDenyDesc = stdCodeInfo.getNm_code();
                            	}
