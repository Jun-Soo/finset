...
<script type="text/javascript">
	...
	function init(orderVal){
		...
		var outVOList = "";
		try{
			inVOJson = JSON.parse('${inVO}');
			outVOList = JSON.parse('${outVOList}');
		}catch(e){
			alert(e);
		}
		...
		for(var j=0; j<outVOList.length; j++){
			...
			if(j == outVOList.length -1){
				...
				for(var i=0; i < gridListAdd.length; i++){
			    	...
			    	html+='						<a href="javascript:void(0);" onclick="goDetail('+"'"+inVOJson.inVOList[parentIdx].mbrCd+"','"+gridListAdd[i].crdtIttCd+"','"+gridListAdd[i].prdtCd+"'"+');">                 '; 
			    	...
			    	html+='							<dl>                                                                                                                 ';
			    	html+='								<dt>기간</dt>                                                                                                                 ';
			    	html+='								<dd>'+parseInt(gridListAdd[i].loanTerm)+'개월</dd>                                                                                                                 ';
			    	html+='							</dl>                                                                                                                 ';
			    	html+='						</div>                                                                                                                    ';
					html+='						<div class="newloan">                                                                                            ';
			    	if(gridListAdd[i].loanAblYn == '0' ){
					    html+='							<span>불가능</span>                                                                                                               ';
			    	}else if(gridListAdd[i].loanAblYn == '1'){
			    		html+='							<span>가능</span>                                                                                                               ';
			    	}else{
			    		html+='							<span>잘못된값</span>                                                                                                               ';	
			    	} 
