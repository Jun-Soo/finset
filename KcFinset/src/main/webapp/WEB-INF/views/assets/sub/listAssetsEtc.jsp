			...
			<c:forEach var="List" items="${pagedList.source}" varStatus="status">
				<div class="container-fluid">
					<h3 class="prdlist-heading">
						${List.assetname} <span class="label-outline">${ufn:getCodeName('cd_etc_type', List.assettype)}</span>
...
<%-- 							<dd class="txt-point">${ufn:formatNumberPattern(List.revenue, "###,###")} 원(${ufn:formatNumberPattern(List.rto_revenue, "###.##")}%)</dd> --%>
						...
						<dl>
							<dt>평가금액</dt>
							<dd>${ufn:formatNumberPattern(List.valueatcur, "###,###")} 원</dd>
						</dl>
						<dl>
							<dt>매수금액</dt>
							<dd>${ufn:formatNumberPattern(List.valattrade, "###,###")} 원 / ${ufn:formatNumberPattern(List.qty, "###,###")}주</dd>
						</dl>
					</div>
				</div>
			</c:forEach>
