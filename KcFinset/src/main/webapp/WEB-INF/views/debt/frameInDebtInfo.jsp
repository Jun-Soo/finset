		...
		<div class="container">
			<form name="frmloanApplyStep" id="frmloanApplyStep" method="post" validate="remove_tag;">
				<div class="area-total">
					<%--등록된 부채가 없으면 신규--%>
					<c:if test="${ debt_y_count == '0'}">
						<span>금융사 등록 부채가 총 <em class="num">${new_debt_count}건</em> 있습니다.</span>
					</c:if>
					<%--등록된 부채가 있으면 신규가 아님--%>
					<c:if test="${ debt_y_count ne '0'}">
						<span>금융사 신규 대출이 총 <em class="num">${debt_y_count}건</em> 있습니다.</span>
					</c:if>
				</div>
				<div class="msg-block">
