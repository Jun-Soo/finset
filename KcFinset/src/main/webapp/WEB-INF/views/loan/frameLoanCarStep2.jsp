...
<script type="text/javascript">
	...
	function scrapCar(){
		$.ajax({
			url : "<c:url value='/m/scrap/scrapCar.crz'/>",
			data : {"car_no" : frmloanApplyStep.car_no.value, "nm_person" : frmloanApplyStep.nm_person.value},
			contentType : "application/x-www-form-urlencoded; charset=UTF-8",
			type : "POST",
			async : false,
			success : function (result) {
...
// 					alert(result.message);
// 				}
				frmloanApplyStep.action = "<c:url value='/m/loan/frameLoanCarStep3.crz'/>";
...
<input type="hidden" name="income_info" id="income_info" value="${income_info }"/>
<input type="hidden" name="loan_code" id="loan_code" value="${loan_code }"/>
<input type="hidden" name="car_no" id="car_no" value="${car_no }"/>
<input type="hidden" name="nm_person" id="nm_person" value="${nm_person }"/>
