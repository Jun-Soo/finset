...
// 			alert("pageCnt:" + pageCnt +":totalPage:"+totalPage);
    ...
    var getCookeyCurTab = function() {
        var curTab = getCookie(LOAN_MORTGAGE_CUR_TAB);
        if(curTab == undefined || curTab == '') {
            curTab = "bank";
            setCookie(LOAN_MORTGAGE_CUR_TAB,curTab);
        }
        return curTab;
    }
	...
	function loanGoodsDetail(cd_fc, cd_goods){
		...
		frmLoanGoodsList.action = "<c:url value='/m/loanhomemortgage/frameLoanHomeMortgageStep3.crz'/>";
		frmLoanGoodsList.submit();
	}
	
	function goCalc(){
		frmLoanGoodsList.action = "<c:url value='/m/loanhomemortgage/frameLoanHomeMortgageStep2.crz'/>";
		frmLoanGoodsList.submit();
	}
...
// 		alert($("input:checkbox[name='goods_choice']:checked").length);
...
// 					alert($(this).find("input[name='test']").val());
//	 				alert(i);
//	 				alert(item);
				}
			});
			...
			frmLoanGoodsList.action = "<c:url value='/m/loanhomemortgage/frameLoanHomeMortgageStep5.crz'/>";
			frmLoanGoodsList.submit();
			
		}
// 		alert($("#listLoanGoods").find("input:checkbox[name='goods_choice']").length);
// 		alert($("#listLoanGoods").find("div[id=loan_product]").length);
		
//  		$("#listLoanGoods").find("input:checkbox[name='goods_choice']").each(function(i, item){			
...
//  		});		
	...
	function goCondition(){
		frmLoanGoodsList.action = "<c:url value='/m/loanhomemortgage/frameLoanHomeMortgageStep4.crz'/>";
		frmLoanGoodsList.submit();
	}
	...
	function loadGoodsTab(type) {
        ...
        setCookie(LOAN_MORTGAGE_CUR_TAB,type);
        // affixBottom('hide');
        ...
        if("affiliates" == type){
			vLoad("listLoanGoods","<c:url value='/m/loanhomemortgage/listLoanAffiliates.crz'/>?orderby="+orderby,data);
		} else {
			vLoad("listLoanGoods","<c:url value='/m/loanhomemortgage/listLoanNoAffiliates.crz'/>?orderby="+orderby,data);
		}
        var page = $("#page").val();
        if(page == 1) {
	...
	function loanGoodsBankDetail(cd_fc, cd_non_goods){
		...
		frmLoanGoodsList.action = "<c:url value='/m/loanhomemortgage/frameLoanHomeMortgageStep3Bank.crz'/>";
		frmLoanGoodsList.submit();
	}
</script>
...
<form method="post" name="frmLoanGoodsList" id="frmLoanGoodsList">
	...
	<input type="hidden" name="cd_goods_class_l" id="cd_goods_class_l" value="02">
	<input type="hidden" name="cd_goods_class_m" id="cd_goods_class_m" value="05, 08">
	...
	<header id="header">
		<div class="input-group">
			...
			<h1>주택담보대출</h1>
			<div class="g-menu">
 				<button type="button" class="ico ico-cal" onclick="goCalc();">시세/대출한도 계산기</button>
 				<button type="button" id="src_menu" class="btn btn-gmenu" onclick="goCondition();">조건선택</button>
			</div>
		</div>
	</header>
	<!-- Content -->
	<section id="content">
		...
		<div id="listLoanGoods">
			<%--<%@ include file="/WEB-INF/views/loanhomemortgage/sub/listLoanAffiliates.jsp"%>--%>
		</div>
		<div class="btn-fixed-bottom affix-bottom" id="next_div">
            <a role="button" class="btn btn-lg btn-disabled btn-block" onclick="loanNextStep();">금리/한도 조회하기</a>
		</div>
	</section>
	<!-- //Content -->
</form>
