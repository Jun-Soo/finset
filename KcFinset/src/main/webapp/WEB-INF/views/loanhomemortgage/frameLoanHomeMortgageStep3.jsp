...
<!-- 						<span class="thumb-logo" style="background-image:url('https://www.kbsavings.com/images/common/logo_header.png');"></span> -->
					<span class="thumb-logo" style="background-image:url('<c:url value='/fincorp/getFinCorpIcon.crz'/>?file_name=${goodsInfo.path_file1}');"></span>
					<!-- <li class="bank-title">${ufn:getNmFc(goodsInfo.cd_fc)}</li> -->
					<h2 class="prd-title">${goodsInfo.nm_goods}</h2>
					<div class="scroll-toggle" id="desc">
				...
				<div class="loan-btn">
					<div class="checkbox ico-loan ico-zzim">
						<input type="checkbox" id="z${goodsInfo.cd_fc}${goodsInfo.cd_goods}" value="Y" onchange="loanGoodsChoice('${goodsInfo.cd_fc}','${goodsInfo.cd_goods}', 'z${goodsInfo.cd_fc}${goodsInfo.cd_goods}', 'Y');" <c:out value="${goodsInfo.yn_favorite eq 'Y' ? 'checked' : ''}"/> ><label class="" for="z${goodsInfo.cd_fc}${goodsInfo.cd_goods}"></label>
					</div>
				</div>
			</div>
		</div>
