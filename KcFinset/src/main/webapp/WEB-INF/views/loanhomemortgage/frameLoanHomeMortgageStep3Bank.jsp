					...
					<div class="list-info">
						...
						<dl>
							...
							<c:if test="${desc_max_limit eq '0'}">
								<c:set var="desc_max_limit" value="-" />
							</c:if>
							<c:if test="${desc_max_limit ne '-'}">
								<c:set var="desc_max_limit" value="최대 ${ufn:formatNumberPattern(desc_max_limit,'###,###.##')}만원" />
							</c:if>
							<c:set var="max_loan_term" value="${goodsInfo.max_loan_term}" />
							<c:if test="${max_loan_term eq ''}">
								<c:set var="max_loan_term" value="-" />
							</c:if>
							<c:if test="${max_loan_term ne '-'}">
								<c:set var="max_loan_term" value="${max_loan_term}년" />
							</c:if>
							<dd>${desc_max_limit} / ${max_loan_term}</dd>
						</dl>
					</div>
					<div class="loan-btn">
