...
<script type="text/javascript">
	...
	$(document).ready(function() {
        setCookie("y_user_end"    ,"");
        setCookie("n_user_end"    ,"");
        setCookie("y_loan_already","");
        setCookie("n_loan_already","");
...
// 	function focusStart(){
// 		frmCertifyStep.nm_person.focus();
// 	}
...
// 							$("#confirmed_certify").attr("disabled", false); //확인버튼 활성화
    ...
    function insertTxFc(){
        ...
        // if(chk == false){ return false;}
		<c:if test="${site eq 'LOCAL'}">
        var ssn_person = frmCertifyStep.ssn_birth.value + frmCertifyStep.ssn2.value;
        console.log("ssn_person="+ssn_person);
        frmCertifyStep.ssn_person.value = ssn_person;
        </c:if>
        var data = $("#frmCertifyStep").serialize();
        if(data == null) return false;
    ...
    function loanApplyStepStep() {
        ...
        frmCertifyStep.action = "<c:url value='/m/loanhomemortgage/frameLoanHomeMortgageStep8.crz'/>";
...
<%-- 						<input type="tel" name="hp" id="hp" class="form-control" maxlength="11" value="${personHp }" readonly="readonly"/> --%>
				...
				<div class="form-group has-feedback" id="cert_no_conteiner">
					...
					<input type="tel" name="smsCertNo" id="smsCertNo" class="form-control"
						   placeholder="인증번호를 입력하세요" autocomplete="off" readonly="readonly" onkeyup="smsCertNoChk();">
