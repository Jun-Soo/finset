...
<script type="text/javascript">
    var listLength = 0;
    <c:if test="${not empty pagedList.source}">
    listLength = "${pagedList.source.size()}";
    </c:if>
		...
		<div class="list-block">
			<div class="container-fluid prd-loan" id="loan_product">
				<a href="#url">
					...
					<div class="list-info">
						<dl class="loan-info" onclick="loanGoodsBankDetail('${List.cd_fc}','${List.cd_non_goods}');">
							<dt>대출금리</dt>
							<dd class="txt-point">
					...
					<div class="loan-btn">
						<div class="checkbox ico-loan ico-zzim">
							<input type="checkbox" id="z${List.cd_fc}${List.cd_non_goods}" value="Y" onchange="loanGoodsChoice('${List.cd_fc}','${List.cd_non_goods}', 'z${List.cd_fc}${List.cd_non_goods}', 'N');" <c:out value="${List.yn_favorite eq 'Y' ? 'checked' : ''}"/> ><label class="" for="z${List.cd_fc}${List.cd_non_goods}"></label>
						</div>
					</div>
				</a>
			</div>
