    ...
    <script type="text/javascript">
        ...
        */
        function kcmRequestCertNo() {
            ...
            // debug("kcmRequestCertNo:data="+data);
        ...
        function loanApplyStepStep() {
            ...
            $('#telComCd').removeAttr("disabled");
            if ( !frmCertifyStep.validateForm() ) return false;
            frmCertifyStep.action = "<c:url value='/m/loanworker/frameLoanWorkerStep8.crz'/>";
...
// debug("confirmedCertify:data="+data);
            if(data == null) return false;
            ...
            $.ajax({
                ...
                async : false,
                success : function (result) {
                    // debug("confirmedCertify:result.result="+result.result);
                    if(result.result == '00') {
                        //submit 전 disable 풀어줘야 넘어감
                        // $('#telComCd').removeAttr("disabled");
                        <%--frmCertifyStep.action = "<c:url value='/m/loanworker/frameLoanWorkerStep7.crz'/>";--%>
...
// 		}
    	...
    	function tmpnextstep() {
    		// $('#telComCd').removeAttr("disabled");
    		frmCertifyStep.action = "<c:url value='/m/loanworker/frameLoanWorkerStep6.crz'/>";
    		frmCertifyStep.submit();
    	}
        ...
        */
        ...
        function testOnload(){
            ...
            $("#telComCd").val("06");
            $("#hp").val("01026111680");
            $("#svcTxSeqno").val("15031654");
            $("#smsCertNo").val("213123");
            smsCertNoChk();
            $("#confirmed_certify").click();
        }
        </c:if>
    </script>
...
<div id="wrapper">
    ...
    <section id="content">
        <form name="frmAcceptTerms" method="post">
        	<div class="container">
	            <div class="check-all">
	                <div class="checkbox" id="dvCheckAll">
                        <label><input type="checkbox" id="checkall" name="checkall" validate="checkbox-required:checkall;label:이용약관;"> 약관 전체 동의</label>
                    </div>
	            </div>
        	</div>
            <!-- panel -->
        ...
        <div class="container">
            <form name="frmCertifyStep" id="frmCertifyStep" method="post" validate="remove_tag;">
                ...
                <input type="hidden" name="svcTxSeqno" id="svcTxSeqno" >
                ...
                <div class="form-block form-input-block-confrim">
                    <div class="form-group">
                        ...
                        <input type="text" name="nm_person" id="nm_person" maxlength="10" class="form-control" value="${nm_person}" validate="trim;strip_ws;label:이름;restrictChars;" autocomplete="off" onkeyup="chkValidate();" readonly/>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="row">
                            <div class="col-xs-5">
                                <input type="number" name="ssn_birth" id="ssn_birth"
                                       ...
                                       placeholder="주민등록번호 앞자리"  onkeyup="ssnBirthChk(); chkValidate();" readonly disabled/>
                    ...
                    <div class="form-group">
						...
						<span class="" aria-hidden="true">
							<select name="telComCd" id="telComCd" validate="select-one;label:통신사" class="selectpicker" data-header="옵션선택">
                                ...
                                <input type="number" name="hp" id="hp" inputmode="numeric" class="form-control" maxlength="11" value="${hp}" />
							</select>
						</span>
                            <span class="form-control-feedback" aria-hidden="true">
                                <a role="button" id="req_certification" class="btn btn-primary btn-cert-no" onclick="kcmRequestCertNo();" href="#">인증요청</a>
                            </span>
                        </div>
                    </div>
                    <div class="form-group has-feedback" id="cert_no_conteiner">
                        <label for="smsCertNo" class="sr-only">인증번호</label>
                        <input type="tel" name="smsCertNo" id="smsCertNo" class="form-control" placeholder="인증번호를 입력하세요" autocomplete="off"
                              onkeyup="smsCertNoChk();">
