...
var FILTER_NUMERIC         		= "[0-9]",
	...
	FILTER_ALPHA_NUMERIC_KOR   	= "[A-Za-z0-9가-힣]";
	FILTER_EMAIL   				= "[A-Za-z0-9_-]";
...
var FormUtility = {
	serialize : function(){
		...
		for (; i<len; i++) {
			...
			switch(el.type) {
				case 'text': case 'password': case 'hidden': case 'textarea': case 'tel':  case 'date':  case 'email':
...
var ElementUtility = {
	...
	required_y_ment : function(msg){
		var label = this.xe_options["label"];
		
		label = label ? label : this.getAttribute("name");
		
	    if ( label ) msg =  "대출 상담사 설명 확인서 " + label + "번 항목이 “예”로 선택되어야 신용조회가 가능합니다";
	    else msg = FAILED_MSG_PREFIX + msg;
            
		return msg;
	},
	
	required_n_ment : function(msg){
		var label = this.xe_options["label"];
		
		label = label ? label : this.getAttribute("name");
		
		if ( label ) msg =  "대출 상담사 설명 확인서 " + label + "번 항목이 “아니오”로 선택되어야 신용조회가 가능합니다";
	    else msg = FAILED_MSG_PREFIX + msg;
...
var filterHangers = {
	...
	"alpha_numeric_kor" : function(){
		return new Worker.Filter(FILTER_ALPHA_NUMERIC_KOR);
	},
	"emailOnly" : function(){
		return new Worker.Filter(FILTER_EMAIL);
	},
...
var validatorHangers = {
	...
	"if-required" : function(fieldNameValue){
		if(fieldNameValue == "" || fieldNameValue == null){
			return;
		}
		var ifValue = fieldNameValue.split("==")[1];
		var fieldName = fieldNameValue.split("==")[0]; 
		if(!fieldName) return;

		if(ifValue == "" || ifValue == null){
			return;
		}
		// 비교 대상이 null이거나 값이 다르면 체크 하지 않음
		// 비교 대상의 값이 매치되는데 확인 대상이 null 이면 false 리턴 
		var validator = new Worker.Validator(),
			self      = this;
			msg       = "필요한 항목이 입력되지 않았습니다.";
		...
		validator.setJob(function(){
			...
			if(el.value.isEmpty()){
				return true;
			}else if(!el.value.isEmpty() && el.value != ifValue){
				return true;
			}else{
				if ((!el || ( !el.value.isEmpty() && el.value == ifValue )) && !value.isEmpty()) {
					return true;
				}else{
					return false;
				}
	...
	"required_y_ment" : function(){
		var validator = new Worker.Validator(),
			self      = this,
			msg       = "";
		validator.setFailedMsg(self.required_y_ment(msg));
		validator.setJob(function(){
			var value   = self.value.replace( /(\s)/g,"");
	...
	"required_n_ment" : function(){
		var validator = new Worker.Validator(),
			self      = this,
			msg       = "";
		validator.setFailedMsg(self.required_n_ment(msg));
		validator.setJob(function(){
			var value   = self.value.replace( /(\s)/g,"");
	...
	"radio-required" : function(fieldName){
		if(!fieldName) return;
		var validator = new Worker.Validator(),
			self      = this,
			msg       = "";
	...
	"checkbox-required" : function(fieldName){
		if(!fieldName) return;
		var validator = new Worker.Validator(),
			self      = this,
			msg       = "";
		...
		validator.setJob(function(){
			if(self.disabled || self.getAttribute("disabled") == "disabled") return true;
			var checkbox = document.getElementsByName(fieldName);
	...
	"email" : function(){
		...
		validator.setJob(function(){
			...
			var regExpEmail = /^[A-Za-z0-9_]+[-A-Za-z0-9_.]*@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z]{2,4}$/;
	...
	"hour" : function(format){
		var validator = new Worker.Validator(),
			self      = this,
			msg       = "시간형식이 아닙니다. 1-24사이의 값을 입력해주세요";		
		
		
		
		validator.setFailedMsg(self.createFailedMsg(msg));
		validator.setJob(function(){
			if(format && format.length > self.value.length){
				for(var i =0, len = format.length; i<len; i++){
					self.value = "0"+self.value;
					if(format.length == self.value.length) break;
				}
			}
			
			var hour = parseInt(self.value, 10);
	...
	"minutes" : function(format){
		var validator = new Worker.Validator(),
			self      = this,
			msg       = "분 형식이 아닙니다. 0-59사이의 값을 입력해주세요";		
		...
		validator.setJob(function(){
			...
			var minutes = parseInt(self.value, 10);
	...
	"restrictChars" : function(){
		var validator = new Worker.Validator(),
			self      = this,
			msg       = "한글, 영어, 숫자만 입력 가능합니다.";
			
		validator.setFailedMsg(self.createFailedMsg(msg));
		validator.setJob(function(){
			var value = self.value;
			if (!value) return true;
			var regExp_restrictChars = /^[a-zA-Z0-9가-힣\s]+$/
			if(!regExp_restrictChars.test(value)) return false;
			return true;
		});
