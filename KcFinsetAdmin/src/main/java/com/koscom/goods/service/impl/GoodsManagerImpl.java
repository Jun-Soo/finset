...
public class GoodsManagerImpl implements GoodsManager {
	...
	public GoodsVO getGoodsRtoInfo(String cd_fc, String cd_goods) {
		...
		goodsInfo.setCd_fc(cd_fc);
		goodsInfo.setCd_goods(cd_goods);
		return goodsMapper.getGoodsRtoInfo(goodsInfo);
	}

	@Override
	@Cacheable("CacheGoodsType")
	public List<GoodsVO> listGoodsInfoBYType(String cd_goods_type) {
		GoodsForm goodsForm = new GoodsForm();
		goodsForm.setCd_goods_type(cd_goods_type);
		return goodsMapper.listGoodsInfoBYType(goodsForm);
	}

	@Override
	@Cacheable("CacheGoodsComp")
	public List<GoodsVO> listGoodsInfoBYComp(String cd_fc) {
		GoodsForm goodsForm = new GoodsForm();
		goodsForm.setCd_fc(cd_fc);
		return goodsMapper.listGoodsInfoBYComp(goodsForm);
	}

	@Override
	public List<GoodsVO> listGoodsInfo(GoodsForm goodsForm) {
		return goodsMapper.listGoodsInfo(goodsForm);
	}

	@Override
	public String getGoodsDetail(String cd_goods, String type) {
		goodsManager = (GoodsManager) SpringApplicationContext.getBean("goodsManager");
	...
	public List<GoodsVO> listCodeGoods(){
		return goodsMapper.listCodeGoods();
	}

	public List<GoodsVO> listGoods(GoodsForm goodsForm){
		return goodsMapper.listGoods(goodsForm);
	}

	@Override
	@CacheEvict(value={"CacheGoods","CacheGoodsType","CacheGoodsComp"}, allEntries=true)
	public ReturnClass clearCacheGoods() {
		logger.info("Cache clear : [CacheGoods]");
		return new ReturnClass(Constant.SUCCESS, "정상 처리하였습니다.");
	}

	@Override
	public int listGoodsCount(GoodsForm goodsForm){
		return goodsMapper.listGoodsCount(goodsForm);
	}

	@Override
	public ReturnClass modifyGoodsDetails(GoodsVO goodsVO) {

		if(1 != goodsMapper.modifyGoodsDetails(goodsVO)){
	...
	public ReturnClass modifyGoodsTransmit(GoodsVO goodsVO) {
		if(workerManager.getCachWorkerApprovalAuth(goodsVO.getId_lst(),"202003")){
			if(1 != goodsMapper.modifyGoodsTransmit(goodsVO)){
				return new ReturnClass(Constant.FAILED, "처리에 실패하였습니다.");
			}
		}
	...
	public ReturnClass createGoods(GoodsVO goodsVO){
		...
		goodsMapper.createGoods(goodsVO);
	...
	public int getGoodsCnt(String cd_goods) {
		return goodsMapper.getGoodsCnt(cd_goods);
	}

	@Override
	public List<GoodsVO> listAgencyGoods(GoodsForm goodsForm) {
		return goodsMapper.listAgencyGoods(goodsForm);
	}

	@Override
	public ReturnClass procAgencyOpen(GoodsVO goodsVO) {
		String[] arrIdAgency = goodsVO.getId_agency().split(",");
		int nCnt = 0;

		for ( String id_agency : arrIdAgency ) {
			goodsVO.setId_agency(id_agency);
			nCnt += goodsMapper.procAgencyGoods(goodsVO);
		}

		if(arrIdAgency.length != nCnt){
			return new ReturnClass(Constant.FAILED, "처리에 실패하였습니다.");
		}
	...
	public ReturnClass procGoodsOpen(GoodsVO goodsVO) {
		String[] arrCdGoods = goodsVO.getCd_goods().split(",");
		int nCnt = 0;

		for ( String cd_goods : arrCdGoods ) {
			goodsVO.setCd_goods(cd_goods);
			nCnt += goodsMapper.procAgencyGoods(goodsVO);
		}

		if(arrCdGoods.length != nCnt){
			return new ReturnClass(Constant.FAILED, "처리에 실패하였습니다.");
		}
	...
	public List<GoodsVO> listGoodsOpenBYType(String id_agency, String cd_goods_type) {
		GoodsForm goodsForm = new GoodsForm();
		goodsForm.setCd_goods_type(cd_goods_type);
		goodsForm.setId_agency(id_agency);
		return goodsMapper.listGoodsOpenBYType(goodsForm);
	}
	...
	public List<GoodsVO> listGoodsOpenBYComp(String id_agency, String cd_fc) {
		...
		goodsForm.setCd_fc(cd_fc);
		goodsForm.setId_agency(id_agency);
		return goodsMapper.listGoodsOpenBYComp(goodsForm);
	}

	@Override
	public List<GoodsVO> listGoodsAgency(String id_agency){
		return goodsMapper.listGoodsAgency(id_agency);
	}

	@Override
	public ReturnClass createGoodsRto(GoodsVO goodsRtoVO){
		int nCnt = goodsMapper.createGoodsRto(goodsRtoVO);
		if ( nCnt == 0 ) {
	...
	public ReturnClass deleteGoods(GoodsVO goodsVO) {
		if (1 != goodsMapper.deleteGoods(goodsVO)) {
	...
	public LinkedHashMap<GoodsVO, List<FinsetInfo>> listGoodsInfoForFinset(GoodsForm goodsForm) {
		...
		String no_bunch = goodsForm.getNo_bunch();

		LinkedHashMap<GoodsVO, List<FinsetInfo>> linkedMap =  new LinkedHashMap<GoodsVO, List<FinsetInfo>>();
		List<GoodsVO> listGoods = goodsMapper.listGoodsInfoForCfs(goodsForm);

		//상품 없을시 강제 리턴
		if(listGoods.size() <= 0){
			logger.info("상품정보 없음");
			List<FinsetInfo> list = new ArrayList<FinsetInfo>();
			FinsetDenyVO finsetDenyVO = new FinsetDenyVO();
			filterDesc = codeManager.getNvlCodeName("FINSET_FILTER", "NOT_EXIST_GOODS_INFO", "상품정보가없음");;
			setDenyDesc(list, filterDesc, finsetDenyVO);
			return linkedMap;
		}

		//고객입력값
		TxFcTransmitVO txFcTransmitVO = new TxFcTransmitVO();
        txFcTransmitVO = loanManager.getTxFcTransmitInfo(no_bunch);
		...
		if(txFcTransmitVO == null){
			...
			List<FinsetInfo> list = new ArrayList<FinsetInfo>();
			FinsetDenyVO finsetDenyVO = new FinsetDenyVO();
			filterDesc = codeManager.getNvlCodeName("FINSET_FILTER", "NOT_EXIST_PERSON_INFO", "개인정보가없음");;
			setDenyDesc(list, filterDesc, finsetDenyVO);
			return linkedMap;
		}

		//상품 cfs 시작
		for (GoodsVO goodsVO : listGoods) {
			...
			//나이 cfs
			if(StringUtil.isNotEmpty(goodsVO.getChk_age_loan())) {
				if(goodsVO.getChk_age_loan().equals("Y")) {
					filterDesc = checkAge(txFcTransmitVO, goodsVO, listFinsetInfo, filterDesc, finsetDenyVO);
				}
			}
			//연소득 cfs
			if(StringUtil.isNotEmpty(goodsVO.getChk_amt_year_income())) {
				if(goodsVO.getChk_amt_year_income().equals("Y")){
					filterDesc = checkAmtYearIncome(txFcTransmitVO, goodsVO, listFinsetInfo, filterDesc, finsetDenyVO);
				}
			}
