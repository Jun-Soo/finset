...
public class Kcb_Segment030 extends AbstractSegment{
	...
	private static final Logger logger = LoggerFactory.getLogger(Kcb_Segment030.class);
	...
	private String[] amt_month_before	= new String[12];	//연체금액KCB_030_INFO       //연체금액;
	private String[] cd_account_before	= new String[12];	//계좌상태코드KCB_030_INFO   //계좌상태코드;
	...
	public HashMap<String, Object> getParseDataByResData(String retStr) throws UnsupportedEncodingException {
		...
		pos+=len; len=9; seq030.setAmt_month_before(StringUtil.getByte2String(bt,pos,len).trim(), 0) ;    seq030.setAmt_month_before1(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=9; seq030.setAmt_month_before(StringUtil.getByte2String(bt,pos,len).trim(), 1) ;    seq030.setAmt_month_before2(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=9; seq030.setAmt_month_before(StringUtil.getByte2String(bt,pos,len).trim(), 2) ;    seq030.setAmt_month_before3(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=9; seq030.setAmt_month_before(StringUtil.getByte2String(bt,pos,len).trim(), 3) ;    seq030.setAmt_month_before4(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=9; seq030.setAmt_month_before(StringUtil.getByte2String(bt,pos,len).trim(), 4) ;    seq030.setAmt_month_before5(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=9; seq030.setAmt_month_before(StringUtil.getByte2String(bt,pos,len).trim(), 5) ;    seq030.setAmt_month_before6(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=9; seq030.setAmt_month_before(StringUtil.getByte2String(bt,pos,len).trim(), 6) ;    seq030.setAmt_month_before7(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=9; seq030.setAmt_month_before(StringUtil.getByte2String(bt,pos,len).trim(), 7) ;    seq030.setAmt_month_before8(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=9; seq030.setAmt_month_before(StringUtil.getByte2String(bt,pos,len).trim(), 8) ;    seq030.setAmt_month_before9(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=9; seq030.setAmt_month_before(StringUtil.getByte2String(bt,pos,len).trim(), 9) ;    seq030.setAmt_month_before10(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=9; seq030.setAmt_month_before(StringUtil.getByte2String(bt,pos,len).trim(), 10) ;   seq030.setAmt_month_before11(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=9; seq030.setAmt_month_before(StringUtil.getByte2String(bt,pos,len).trim(), 11) ;   seq030.setAmt_month_before12(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=1; seq030.setCd_account_before(StringUtil.getByte2String(bt,pos,len).trim(), 0) ;   seq030.setCd_account_before1(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=1; seq030.setCd_account_before(StringUtil.getByte2String(bt,pos,len).trim(), 1) ;   seq030.setCd_account_before2(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=1; seq030.setCd_account_before(StringUtil.getByte2String(bt,pos,len).trim(), 2) ;   seq030.setCd_account_before3(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=1; seq030.setCd_account_before(StringUtil.getByte2String(bt,pos,len).trim(), 3) ;   seq030.setCd_account_before4(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=1; seq030.setCd_account_before(StringUtil.getByte2String(bt,pos,len).trim(), 4) ;   seq030.setCd_account_before5(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=1; seq030.setCd_account_before(StringUtil.getByte2String(bt,pos,len).trim(), 5) ;   seq030.setCd_account_before6(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=1; seq030.setCd_account_before(StringUtil.getByte2String(bt,pos,len).trim(), 6) ;   seq030.setCd_account_before7(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=1; seq030.setCd_account_before(StringUtil.getByte2String(bt,pos,len).trim(), 7) ;   seq030.setCd_account_before8(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=1; seq030.setCd_account_before(StringUtil.getByte2String(bt,pos,len).trim(), 8) ;   seq030.setCd_account_before9(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=1; seq030.setCd_account_before(StringUtil.getByte2String(bt,pos,len).trim(), 9) ;   seq030.setCd_account_before10(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=1; seq030.setCd_account_before(StringUtil.getByte2String(bt,pos,len).trim(), 10) ;  seq030.setCd_account_before11(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=1; seq030.setCd_account_before(StringUtil.getByte2String(bt,pos,len).trim(), 11) ;  seq030.setCd_account_before12(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=2; seq030.setGoods_code(StringUtil.getByte2String(bt,pos,len).trim()) ;
		pos+=len; len=3; seq030.setLoan_mount(StringUtil.getByte2String(bt,pos,len).trim()) ;

		pos+=len; len=41;	//41 ==> 46 //김현구
	...
	public String getAmt_month_before(int i) {
		return amt_month_before[i];
	}

	public void setAmt_month_before(String[] amt_month_before) {
		this.amt_month_before = amt_month_before;
	}

	public void setAmt_month_before(String amt_month_before, int i) {
		this.amt_month_before[i] = amt_month_before;
	}

	public String getCd_account_before(int i) {
		return cd_account_before[i];
	}
	...
	public void setCd_account_before(String cd_account_before, int i) {
		this.cd_account_before[i] = cd_account_before;
	}
