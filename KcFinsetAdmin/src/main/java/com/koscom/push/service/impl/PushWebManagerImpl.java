package com.koscom.push.service.impl;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

import com.koscom.push.model.PushForm;
import com.koscom.push.model.PushVO;
import com.koscom.push.service.PushManager;
import com.koscom.push.service.PushWebManager;
import com.koscom.util.ReturnClass;

@Service("pushWebManager")
public class PushWebManagerImpl implements PushWebManager{
	@Autowired
	PushManager pushManager;
	@Override
	public List<PushVO> listPushInfo(PushForm eventForm) {
		return pushManager.listPushInfo(eventForm);
	}
	@Override
	public ReturnClass createPushInfo(PushVO pushVO) {
		return pushManager.createPushInfo(pushVO);
	}
	@Override
	public int listPushInfoCount(PushForm pushForm) {
		return pushManager.listPushInfoCount(pushForm);
	}
	@Override
	public PushVO viewPushInfo(PushVO pushVO) {
		return pushManager.viewPushInfo(pushVO);
	}
	@Override
	public List<PushVO> viewPushEachInfoPerson(PushVO pushVO) {
		return pushManager.viewPushEachInfoPerson(pushVO);
	}
	@Override
	public int getNewSeqPush(PushForm pushForm) {
		return pushManager.getNewSeqPush(pushForm);
	}
	@Override
	public ReturnClass createPushEachInfo(PushVO pushVO) {
		return pushManager.createPushEachInfo(pushVO);
	}
}
