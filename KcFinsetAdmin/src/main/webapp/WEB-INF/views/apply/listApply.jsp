...
<%-- 	              <td><a href="#" onclick="goGoodsDetailsPop('${List.cd_fc}','${List.cd_goods}');"><strong>${ufn:getGoodsDetail(List.cd_goods, '')}</strong></a></td>						   <!-- 상품명 --> --%>
...
<div class="panel-footer">
	<span class="pull-left">
		<span class="total-num em"><span></span>총건수 : <em>${ufn:formatNumber(pagedList.recordCount)} 건</em></span>
			...
			<span class="total-num em"><span></span>신청금액 : <em>${ufn:formatNumber(pagedList.amtSumMap['AMTTOTAPPLY'])} 원</em></span>
		</c:if> 
		<%-- <c:if test="${!empty pagedList.amtSumMap['AMTTOTAPPR']}">
			<span class="total-num em"><span></span>승인금액 : <em>${ufn:formatNumber(pagedList.amtSumMap['AMTTOTAPPR'])} 원</em></span>
		</c:if> --%>
	</span>
</div>
