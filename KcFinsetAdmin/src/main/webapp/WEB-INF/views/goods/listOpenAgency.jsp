...
$(document).ready(function () {
	...
	window.setupValidateForm( frmList );
});

function procAgencyGoods(yn_open){
	if ( countCheckBox(frmList.cd_goods_list) == 0 ) {
		alert("선택된 대상이 없습니다.");
		return false;
	}
	
	frmList.cd_goods.value = getRadioOption(frmList.cd_goods_list);
	...
	$.ajax({
		url : "<c:url value='/goods/procGoodsOpen.json'/>",
		data : data,
...
<div class="srch">
	<div class="form-group" id="sandbox-container">
		<!-- 제휴사 -->
		<select name="id_agency" class="selectpicker">
			${ufn:makeAgencyOptions("매체사명", goodsForm.id_agency)}
		</select>
...
<!-- List -->
<table id="tbl_listAgency" class="table table-bordered tbl-info" cellspacing="0" width="100%">
	...
	<thead>
         <tr>
            <th> <input type="checkbox" id="checkAll" onclick="checkBoxAllCheck(this,'cd_goods_list');"/><label for="checkAll" class="checkbox-inline"></label></th>
            <th> 매체사명</th>
            <th> 상품명</th>
            <th> 노출여부</th>
        </tr>
	...
	<tbody>
    	...
    	<c:choose>
			<c:when test="${goodsForm.id_agency == ''}">
				<tr>
			...
			<c:otherwise>
				${ufn:showOpenGoodsAgency(goodsForm.tab_type, goodsForm.id_agency)}
			</c:otherwise>
		</c:choose>
	</tbody>
</table>
