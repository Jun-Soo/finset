...
<input type="hidden" name="listLength" value="${fn:length(listStatAdvertisementApply)}" />
	...
	<c:forEach var="List" items="${listStatAdvertisementApply}" >
		...
		<c:if test="${trCnt2 eq rowCnt2}">
			<tr class="active">
				<td></td>
				<td></td>
				<td class="right">${ufn:formatNumber(semi_sum_cnt_apply)}</td>
				<td class="right">${ufn:formatNumber(semi_sum_cnt_approval)}</td>
				<td class="right">${ufn:formatNumber(semi_sum_amt_approval)}</td>
				<c:choose>
					<c:when test="${semi_sum_cnt_apply eq 0}">
						<td class="right">100.00%</td>
					</c:when>
					<c:otherwise>
						<td class="right"><fmt:formatNumber value="${semi_sum_cnt_approval / semi_sum_cnt_apply}" pattern="0.00%"/></td>
					</c:otherwise>
				</c:choose>
				<td class="right">${ufn:formatNumber(semi_sum_cnt_approval_500)}</td>	
				<td class="right">${ufn:formatNumber(semi_sum_cnt_approval_1000)}</td>
				<td class="right">${ufn:formatNumber(semi_sum_cnt_approval_1001)}</td>
				<td class="right">${ufn:formatNumber(semi_sum_amt_approval_500)}</td>
				<td class="right">${ufn:formatNumber(semi_sum_amt_approval_1000)}</td>
				<td class="right">${ufn:formatNumber(semi_sum_amt_approval_1001)}</td>
				<td class="right">${ufn:formatNumber(semi_sum_cnt_reject)}</td>
