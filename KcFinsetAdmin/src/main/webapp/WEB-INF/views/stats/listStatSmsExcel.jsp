...
<table>
	...
	<c:forEach var="List" items="${listStatSms}">
		...
		<tr>
			...
			<td>${ufn:formatNumber(List.cnt_sms)}</td>
			<td>${ufn:formatNumber(List.cnt_sms_sending)}</td>
			<td>${ufn:formatNumber(List.cnt_sms_success)}</td>
			<td>${ufn:formatNumber(List.cnt_sms_fail)}</td>
			<c:choose>
				<c:when test="${List.cnt_sms eq 0}">
					<td>-</td>
				</c:when>
				<c:otherwise>
					<td><fmt:formatNumber value="${List.cnt_sms_success/List.cnt_sms}" pattern="0.00%"/></td>
				</c:otherwise>
			</c:choose>
		</tr>
		...
		<c:if test="${trCnt eq rowCnt}">			
			<tr>
				<td>소계</td>
				<td></td>
				<td>${ufn:formatNumber(sum_cnt_sms)}</td>
				<td>${ufn:formatNumber(sum_cnt_sending)}</td>
				<td>${ufn:formatNumber(sum_cnt_success)}</td>
				<td>${ufn:formatNumber(sum_cnt_fail)}</td>
				<c:choose>
					<c:when test="${sum_cnt_sms eq 0}">
						<td>-</td>
					</c:when>
					<c:otherwise>
						<td><fmt:formatNumber value="${sum_cnt_success/sum_cnt_sms}" pattern="0.00%"/></td>
					</c:otherwise>
				</c:choose>	
			</tr>
	...
	<c:if test="${!empty listStatSms}">
	 	<tr>
			...
			<td></td>
			<td>${ufn:formatNumber(all_sum_cnt_sms)}</td>
			<td>${ufn:formatNumber(all_sum_cnt_sending)}</td>
			<td>${ufn:formatNumber(all_sum_cnt_success)}</td>
			<td>${ufn:formatNumber(all_sum_cnt_fail)}</td>
			<c:choose>
				<c:when test="${all_sum_cnt_sms eq 0}">
					<td>100.00%</td>
				</c:when>
				<c:otherwise>
					<td><fmt:formatNumber value="${all_sum_cnt_success / all_sum_cnt_sms}" pattern="0.00%"/></td>
				</c:otherwise>
			</c:choose>
		</tr>
	</c:if>
