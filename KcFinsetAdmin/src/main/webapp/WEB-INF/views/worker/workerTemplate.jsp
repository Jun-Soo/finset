...
<c:if test="${!empty codeVO.code_value}">
<div class="alert alert-warning alert-dismissible exclamation-list" role="alert">
	...
	<ul>
		<li>권한그룹 목록을 확인하고 권한그룹을 변경 할 수 있습니다.</li>
		<li><strong>${ufn:getCodeName("cd_template_group", codeVO.code_value)}</strong> 권한그룹을 수정하고 있습니다.</li>
	</ul>
</div>
<div class="panel panel-default panel-sub toggle-panel">
	...
	<div class="panel-collapse toggle-cont">
		<form name="frmProgramTemplate" id="frmProgramTemplate">
		<input type="hidden" name="cd_template_group" value="${codeVO.code_value}">
		...
		<section class="tree-nav">
			<div class="tree-row">
				...
				<div class="tree-cell tree-btn">
					<button type="button" class="btn add" onclick="movePmTemplate('ADD')"><span class="glyphicon glyphicon-arrow-right" aria-hidden="true"></span><strong>추가</strong></button>
					<button type="button" class="btn del" onclick="movePmTemplate('SUB')"><span class="glyphicon glyphicon-arrow-left" aria-hidden="true"></span><strong>삭제</strong></button>
				</div>
				<div class="tree-cell" id="template_program">
				  <ul>
				  	<c:forEach var="main_program" items="${programTemplateList}" varStatus="status">
				  	<c:if test="${ufn:isProgram(main_program.id_program,'','00','00')}">
				    <li id="${main_program.id_program}" data-jstree='{"opened":true}'>${ufn:getProgram(main_program.id_program,'NM')}
				      <ul>
				      	<c:forEach var="sub_program" items="${programTemplateList}">
				        ...
				        <li id="${sub_program.id_program}" data-jstree='{"icon":"glyphicon glyphicon-folder-open"}'>${ufn:getProgram(sub_program.id_program,'NM')}
					        <ul>
					      	<c:forEach var="child_program" items="${programTemplateList}">
